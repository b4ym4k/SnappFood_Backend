...
func ManagerCreateFood(c *fiber.Ctx) error {
	...
	//
	//database.DB.Where("id = ?", claims.Issuer).Model(&restaurant).Update("food_name", restaurant.Food)
	//database.DB.Where("id = ?", claims.Issuer).Model(&restaurant).Update("price", restaurant.Food)
	//
	//return c.JSON(restaurant)
